...
class ConnectionManager:
    def __init__(self, agent_config):
        """Initialize the connection manager with agent configuration
        
        Args:
            agent_config: List or dictionary containing connection configurations
        """
        self.connections: Dict[str, BaseConnection] = {}
        if isinstance(agent_config, list):
            for config in agent_config:
                self._register_connection(config)
        elif isinstance(agent_config, dict) and "connections" in agent_config:
            for config in agent_config["connections"]:
                self._register_connection(config)
